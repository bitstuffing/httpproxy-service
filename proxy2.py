...
#to get working httpproxy with kodi
import logger
import xbmcaddon
import xbmcplugin
import xbmcgui
...
addon = xbmcaddon.Addon(id='org.harddevelop.kodi.proxy')
DEFAULT_PORT = int(addon.getSetting("port"))
REDIRECT = bool(addon.getSetting("local_redirect")=="true")
SERVER = "127.0.0.1"
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    #it could be used with https, but setup_https_intercept.sh needs to be called
    cakey = 'ca.key'
    cacert = 'ca.crt'
    certkey = 'cert.key'
    certdir = 'certs/'
    ...
    def check_blacklist(self,path):
        state = False
        path = xbmcaddon.Addon(id='org.harddevelop.kodi.proxy').getAddonInfo('path')+"/"
        f = open(path+"blacklist.txt","r+")
        for line in f.readlines():
            if path.find(line)>-1:
                logger.debug(path+" - "+line)
                state = True
                logger.debug(with_color(32, "==== DETECTED in BLACKLIST ====\n%s\n" % path))
        f.close()
        return state
    ...
    def do_GET(self):
        if self.path == 'http://proxy2.test/':
            ...
            logger.debug(with_color(32, "==== BANNED by BLACKLIST ====\n%s\n" % self.path))
            return
        else:
            logger.debug(with_color(16, "==== ALLOWED ==== \n%s\n" % self.path))
            req = self

        for req_header in req.headers:
            logger.debug("Original Header: "+req_header)
        ...
        for simple_header in req_headers:
            logger.debug("Final Request Header: "+str(simple_header))
        ...
        if res_body_modified is not None:
            ...
            logger.debug("content length: "+str(len(res_body)))

        res_headers = self.filter_headers(res.headers)

        for simple_header in res_headers:
            logger.debug("simple response: "+str(simple_header))
    ...
    def print_info(self, req, req_body, res, res_body):
        ...
        if u.query:
            ...
            logger.debug(with_color(32, "==== QUERY PARAMETERS ====\n%s\n" % query_text))

        cookie = req.headers.get('Cookie', '')
        if cookie:
            cookie = parse_qsl(re.sub(r';\s*', '&', cookie))
            logger.debug(with_color(32, "==== COOKIE ====\n%s\n" % cookie))

        auth = req.headers.get('Authorization', '')
        if auth.lower().startswith('basic'):
            token = auth.split()[1].decode('base64')
            logger.debug(with_color(31, "==== BASIC AUTH ====\n%s\n" % token))
        ...
        if req_body is not None:
            ...
            if req_body_text:
                logger.debug(with_color(32, "==== REQUEST BODY ====\n%s\n" % req_body_text))
        ...
        if cookies:
            cookies = '\n'.join(cookies)
            logger.debug(with_color(31, "==== SET-COOKIE ====\n%s\n" % cookies))
        ...
        if res_body is not None:
            ...
            if content_type.startswith('application/json'):
                ...
                if m:
                    h = HTMLParser()
                    logger.debug(with_color(32, "==== HTML TITLE ====\n%s\n" % h.unescape(m.group(1).decode('utf-8'))))
            elif content_type.startswith('text/') and len(res_body) < 1024:
                res_body_text = res_body

            if res_body_text:
                logger.debug(with_color(32, "==== RESPONSE BODY ====\n%s\n" % res_body_text))
    ...
    def request_handler(self, req, req_body):
        ...
        if req.path.find("|")>-1 or req.path.find(".ts")>-1 or req.path.find("http://cdn.sstream.pw/")>-1:
            ...
            if req.path.find("|")>-1:
                newPath = req.path[:req.path.find("|")]
            elif req.path.find("?original-request=")>-1:
                req.path = req.path[req.path.find("?original-request=")+len("?original-request="):]
            ...
            logger.debug("new path: "+req.path)
        else:
            logger.debug("normal request, not processed for "+req.path)
            ...
            for line in res_body.split("\n"):
                if line.find(".ts")==len(line)-len(".ts"):
                    ...
                    if line.find("://")==-1: #needs the domain fix
                        line = path+"/"+line
                    if req.path.find("http://cdn.sstream.pw/")>-1 and REDIRECT:
                        line = "http://"+SERVER+":"+str(DEFAULT_PORT)+"?original-request="+line
                    logger.debug("new line: "+line)
                else:
                    logger.debug(str(len(line))+": "+line)
                content += line+"\n"
            res_body = content
            logger.debug(with_color(32, "==== UPDATED RESPONSE #EXTINF BODY ====\n%s\n" % res_body))
        return res_body
